...
var generateFortuneCookie = function() {
  ...
  var fortuneCookieContainer = document.getElementById('fortune-cookie-text');
  ...
  newListItem.innerHTML = fortuneCookieContainer.innerHTML;
  
   // 3. Select the `#previous-fortunes-container` container and use
        //    `appendChild()` to append the new `li` element you created above.
  var previousFortune = document.getElementById('previous-fortunes-container');
  previousFortune.appendChild(newListItem);
  ...
  fortuneCookieContainer.innerHTML = fortunesList[Math.floor(Math.random() * fortunesList.length)];
  fortunesList.splice();
